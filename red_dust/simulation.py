...
class SimulationController:
    ...
    def step(self) -> bool:
        ...
        game_over = self.env.step(actions, agent_names)
    ...
    def time_travel(self, target_tick: int) -> bool:
        ...
        if target_tick < 0 or target_tick >= len(self.history):
            print(
                f"Cannot rewind to tick {target_tick}. History size: {len(self.history)}"
            )
            return False
        ...
        if historical_state.rng_state:
            ...
            print(
                f"WARNING: No RNG state saved for tick {target_tick}. Creating new RNG with seed {self.seed + target_tick}"
            )
    ...
    def get_current_status(self) -> dict:
        ...
        return {
            ...
            "agents_alive": sum(
                1 for a in self.env.state.agents.values() if a.is_alive()
            ),
