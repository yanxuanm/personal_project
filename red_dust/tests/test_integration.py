...
def test_time_travel_consistency(client: TestClient):
    ...
    def advance_ticks(num_ticks: int):
        for _ in range(num_ticks):
            ...
            assert response.status_code == 200, f"Failed to advance tick: {response.text}"
    
    # Helper function to rewind to tick
    def rewind_to(tick: int):
        response = client.post(f"/api/rewind/{tick}")
        assert response.status_code == 200, f"Failed to rewind to tick {tick}: {response.text}"
    
    # Setup: Reset simulator with known seed and LLM disabled
    reset_response = client.post("/api/reset", params={"seed": 42, "use_llm": False})
    assert reset_response.status_code == 200, f"Failed to reset simulator: {reset_response.text}"
    ...
    if state_a.get('logs') and state_b.get('logs'):
        ...
        assert logs_a == logs_b, \
            f"Recent logs mismatch. A: {logs_a}, B: {logs_b}"
    ...
    for resource in state_a['resources']:
        if resource in state_c['resources']:
            ...
            if diff > 0.01:
                ...
                print(f"  Resource {resource} changed: {state_a['resources'][resource]:.2f} -> {state_c['resources'][resource]:.2f}")
                break
    ...
    if not changes_detected and state_c.get('logs'):
        ...
        if len(state_c['logs']) > len(state_a.get('logs', [])):
            changes_detected = True
            print(f"  New logs added: {len(state_c['logs']) - len(state_a.get('logs', []))} new entries")
            ...
            if agent_name in state_c['agents']:
                ...
                if 'health' in agent_a and 'health' in agent_c:
                    if abs(agent_a['health'] - agent_c['health']) > 0.01:
                        changes_detected = True
                        print(f"  Agent {agent_name} health changed: {agent_a['health']:.2f} -> {agent_c['health']:.2f}")
                        break
                ...
                if 'location' in agent_a and 'location' in agent_c:
                    if agent_a['location'] != agent_c['location']:
                        changes_detected = True
                        print(f"  Agent {agent_name} location changed: {agent_a['location']} -> {agent_c['location']}")
                        break
    
    assert changes_detected, "No changes detected after advancing 1 tick. Simulation may be stuck."
    ...
    for field in ['tick', 'resources', 'agents', 'game_over']:
        if field == 'resources':
            for resource in state_at_10['resources']:
                ...
                diff = abs(state_at_10['resources'][resource] - state_d['resources'][resource])
                assert diff < 0.01, \
    ...
    print("✓ Deterministic continuation verified: Same timeline reproduced after rewind")
...
def test_deterministic_simulation(client: TestClient):
    ...
    reset1 = client.post("/api/reset", params={"seed": 100, "use_llm": False})
    ...
    reset2 = client.post("/api/reset", params={"seed": 100, "use_llm": False})
    ...
    for resource in resources1:
        ...
        assert diff < 0.01, f"Resource {resource} mismatch: {resources1[resource]} vs {state2['resources'][resource]}"
    ...
    for agent_name in agents1:
        ...
        assert diff < 0.01, f"Agent {agent_name} health mismatch: {health1} vs {health2}"
